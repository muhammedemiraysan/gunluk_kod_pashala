...
void loop() {
  ...
  sagx = map(ch1Value,-100,100,0,1024);
  sagy = map(ch2Value,-100,100,0,1024);

  soly = map(ch3Value,-100,100,0,1024);
  solx = map(ch4Value,-100,100,0,1024);

if(sagx > 507 && sagx < 518){sagx = 500;}
if(sagy > 507 && sagy < 518){sagy = 500;}
  if(solx > 490 && solx < 550){solx = 500;}if(soly > 490 && soly < 550){soly = 500;}
